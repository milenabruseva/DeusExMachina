...
def write_dict_to_file(agent):
    ...
    with open("q_table.json", "w") as output:
        output.write(json.dumps(agent.q1_table))
        output.close()
...
def read_dict_from_file(agent):
    ...
    with open("q_table.json", "r") as input:
        agent.q1_table = json.load(input)
        input.close()
...
def act(agent, state: dict):
    ...
    agent.game_state = old_state_dict_from_new(agent, state)
    ...
    if agent.game_state["step"] == 1 and agent.radius is None:
        training_radius(agent) if s.SCENARIOS["classic"]["CRATE_DENSITY"] == 0 else real_radius(agent)
    ...
    if agent.train_flag:  # This flag is our own flag, that acknowledge us that this is training.
        action_rewards = np.array(agent.q1_table[string])
        ...
        #  "Exploration-Exploitation".
            ...
            action = np.argmax(agent.q1_table[string])
    ...
    # When not in training mode, we should use the pre-prepared Q table.
        ...
        try:
            ...
            # playing.
            knowledge_list = np.array(agent.q1_table[stateToStr(agent.curr_state)])
...
def find_state(agent):
    ...
    try:
        agent.q1_table[string_state]
    except Exception as ex:
        agent.q1_table[string_state] = list(np.zeros(6))
...
def reward_update(agent):
    ...
    try:
        ...
        Q0sa0 = agent.q1_table[prev_state_string][agent.prev_action]
        Qs1a1 = agent.rewards["dead"] if GOT_KILLED == agent.events[-1] else max(agent.q1_table[state_string])
        curr_reward = Q0sa0 + a * (agent.prev_reward + g * Qs1a1 - Q0sa0)
        agent.q1_table[prev_state_string][agent.prev_action] = curr_reward
