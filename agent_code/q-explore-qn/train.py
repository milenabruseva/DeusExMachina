...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    q_update = reward + self.gamma *\
               max_exploration_function(self, self.qn_table[new_state_str])
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if (last_game_state["round"] % self.save_n_rounds) == 0:
        with open(self.qn_table_filename, "w") as qn_table_file:
            qn_table = self.qn_table
            qn_table["meta"] = {"algorithm": ALGORITHM, "feature": self.feature, "q_table_id": self.qn_table_id}
            json.dump(qn_table, qn_table_file, indent=4, sort_keys=True)
...
def max_exploration_function(self, qn_values):
    max_value = qn_values[0] + self.exp / qn_values[len(self.ACTIONS)]
    for i in range(1,6):
        value = qn_values[i] + self.exp / qn_values[i + len(self.ACTIONS)]
        if max_value < qn_values[i] + self.exp / qn_values[i + len(self.ACTIONS)]:
            max_value = value
    return max_value
